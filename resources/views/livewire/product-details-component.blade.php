            ...
            <div class="col-lg-9 col-md-8 col-sm-8 col-xs-12 main-content-area">
                <div class="wrap-product-detail">
                    ...
                    <div class="detail-info">
                        <div class="product-rating">
                            ...
                            if ($num_of_review > 0){
                                foreach($product->orderItems->where('rstatus',1) as $orderItem){
                                    //echo $orderItem->review->rating."<br />";
                                    $avg_rating = $avg_rating + $orderItem->review->rating;
                                }
                                //$avg_rating = floor($avg_rating / $num_of_review);
                                $avg_rating = $avg_rating / $num_of_review;
                                //echo $avg_rating;
                            }  
                            @endphp
                            @for ($i=1; $i<=5; $i++)
                            ...
                            <a href="#" class="count-review">({{ $num_of_review  }})</a>
                        </div>
                        ...
                        @if (($product->sale_price > 0) && ($sale->status == 1) && ($sale->sale_date > Carbon\Carbon::now('Asia/Dhaka')))
                            ...
                            <div class="tab-content-item " id="review">                              
                                <div class="wrap-review-form">
                                    ...
                                    <div id="comments">
                                        @php
                                        if($num_of_review > 0){
                                            $num_of_review_padded = sprintf("%02d", $num_of_review);
                                        }else{
                                            $num_of_review_padded = 0;
                                        }
                                            //echo $num_of_review_padded; // returns 04
                                        @endphp
                                        <h2 class="woocommerce-Reviews-title">{{ $num_of_review_padded }} review for <span>{{ $product->name }}</span></h2>
                                        <ol class="commentlist">
                                            @foreach ($product->orderItems->where('rstatus',1) as $orderItem)
                                            <li class="comment byuser comment-author-admin bypostauthor even thread-even depth-1" id="li-comment-20">
                                                <div id="comment-20" class="comment_container"> 
                                                    @if (!empty($orderItem->order->user->profile->image))
                                                        <img alt="{{$orderItem->order->user->name}}" src="{{ asset('assets/images/profile/') }}/{{ $orderItem->order->user->profile->image }}" height="80" width="80">
                                                    @else
                                                        <img alt="User" src="{{ asset('assets/images/author-avata.jpg') }}" height="80" width="80">
                                                    @endif
